...
```
java -jar sprite-utils.jar \
draw \
--dir /path/to/open-eggbert-legacy-assets/speedy_eggbert_1/IMAGE08 \
--rectangle-color 255,0,0 \
--sprite-sheet-path /path/to/open-eggbert/assets/default-spritesheets/speedy_blupi_I.spritesheet.csv \
--row 5 \
...
```
java -jar sprite-utils.jar draw --dir /path/to/speedy_eggbert_1/IMAGE08 --rectangle-color 0,255,0 --sprite-sheet-path /path/to/speedy_blupi_I.spritesheet.csv --row 4 --file-name BLUPI000.BLP
```

### How to run Speedy Blupi/Eggbert in window mode on Linux without blue backgrounds

```
cd /path/to/speedy_eggbert_1 ; Xephyr :1 -ac -screen 800x600x16 & DISPLAY=:1 xfwm4 & DISPLAY=:1 wine Eggbert1.exe
```
