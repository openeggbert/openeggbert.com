---
title: Projects
breadcrumb:
- label: Home
  href: index.html
- label: Projects
  href: Projects/index.html
subpages:
- label: Go Up
  href: ./index.html
- label: Free Eggbert
  href: Projects/Free_Eggbert/index.html
- label: Mobile Eggbert
  href: Projects/Mobile_Eggbert/index.html
- label: Planet Blupi (fork)
  href: Projects/Planet_Blupi/index.html
- label: Galaxy Eggbert
  href: Projects/Galaxy_Eggbert/index.html
- label: Sharp Runtime
  href: Projects/Sharp_Runtime/index.html
- label: CNA
  href: Projects/CNA/index.html
- label: Free Direct
  href: Projects/Free_Direct/index.html
- label: Free Api
  href: Projects/Free_Api/index.html
- label: EasyGL
  href: Projects/EasyGL/index.html
- label: MetaGL
  href: Projects/MetaGL/index.html
- label: MeshCraft
  href: Projects/MeshCraft/index.html
- label: Sprite Utils
  href: Projects/Sprite_Utils/index.html
- label: Youtube Frontend
  href: Projects/Youtube_Frontend/index.html
---

## Active projects

| Name | Website | Status | Description |
| --- | --- | --- | --- |
| [Free Eggbert](Projects/Free_Eggbert/index.html) | [freeeggbert.openeggbert.com](https://freeeggbert.openeggbert.com) |  | Revival of Speedy Blupi (Epsitec) |
| [Mobile Eggbert](Projects/Mobile_Eggbert/index.html) | [mobileeggbert.openeggbert.com](https://mobileeggbert.openeggbert.com) |  | Revival of Speedy Blupi (Windows Phone) |
| [Planet Blupi (fork)](Projects/Planet_Blupi/index.html) | [planetblupi.openeggbert.com](https://planetblupi.openeggbert.com) |  | Fork of Planet Blupi running on Free API and Free Direct |
| [Galaxy Eggbert](Projects/Galaxy_Eggbert/index.html) | [galaxyeggbert.openeggbert.com](https://galaxyeggbert.openeggbert.com) |  |  |
| [Sharp Runtime](Projects/Sharp_Runtime/index.html) | [sharpruntime.openeggbert.com](https://sharpruntime.openeggbert.com) |  |  |
| [CNA](Projects/CNA/index.html) | [cna.openeggbert.com](https://cna.openeggbert.com) / [libcna.com](https://libcna.com) |  | XNA-like C++ library using SDL 3 |
| [Free Direct](Projects/Free_Direct/index.html) | [freedirect.openeggbert.com](https://freedirect.openeggbert.com) |  | DirectX 3-like wrapper around CNA |
| [Free API](Projects/Free_Api/index.html) | [freeapi.openeggbert.com](https://freeapi.openeggbert.com) |  | Reimplementation of Windows API subset |
| [EasyGL](Projects/EasyGL/index.html) | [easygl.openeggbert.com](https://easygl.openeggbert.com) |  |  |
| [MetaGL](Projects/MetaGL/index.html) | [metagl.openeggbert.com](https://metagl.openeggbert.com) |  |  |
| [MeshCraft](Projects/MeshCraft/index.html) | [meshcraft.openeggbert.com](https://meshcraft.openeggbert.com) |  |  |
| [Sprite Utils](Projects/Sprite_Utils/index.html) |  |  |  |
| [Youtube Frontend](Projects/Youtube_Frontend/index.html) |  |  |  |

## Cancelled projects (superseded by C++ rewrite)

| Name | Description |
| --- | --- |
| Mobile Eggbert MonoGame Android | C# MonoGame port for Android |
| Mobile Eggbert MonoGame Desktop | C# MonoGame port for desktop |
| Mobile Eggbert FNA Desktop | C# FNA port for desktop |
| Mobile Eggbert KNI Web | C# KNI port for web browser (was working, superseded by C++ version) |
